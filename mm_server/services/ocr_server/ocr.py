# code from: https://github.com/netease-youdao/QAnything/
import copy
import math
import os
import sys
...
class TextDetector(object):
    ...
    def __call__(self, img):
        ...
        for i in range(100000):
            try:
                outputs = self.predictor.run(None, input_dict)  # (b,1, 320, 512)
                break
            except Exception as e:
                if i >= 3:
                    raise e
...
class OCRResponse(BaseModel):
    ...
    image_width: int
    image_height: int
    text_proportion: float  # 文字占比


class OCQanythingRModel(object):
    ...
    def __call__(self, img, cls=True) -> List[OCRResponse]:
        time_dict = {"det": 0, "rec": 0, "cls": 0, "all": 0}
        ...
        ocr_resps = []
        for index, (box, rec_result) in enumerate(zip(dt_boxes, rec_res)):
            img_crop = img_crop_list[index]
            box_h, box_w, _c = img_crop.shape
            ...
            resp = OCRResponse(
                ...
                text_proportion=round(box_area / (img_w * img_h * 1.0), 3),
            )
            ocr_resps.append(resp)
        end = time.time()
        time_dict["all"] = end - start
        print(f"ocr running time: {time_dict}")
        # return [item[0] for item in list(filter_rec_res)]
        return ocr_resps


if __name__ == "__main__":
    ocr = OCQanythingRModel(model_dir="ocr_models", device="cpu")
